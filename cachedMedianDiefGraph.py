...
data['rawMedianDay_ts'] = open_file('testData/cachedData/onRawDataMedianDay_dief.txt')
data['summariesMedianDay_ts'] = open_file('testData/cachedData/summariesWeekMedianDayData_dief.txt')
...
st = fig.suptitle("Diefficiency comparison between server side\n and client side median summaries for cached fragments", fontsize="17")
...
axs[0, 0].set_title('Median per minute', size=15)
...
axs[0, 1].set_title('Median per minute', size=15)
...
axs[1, 0].set_title('Median per hour', size=15)
...
axs[1, 1].set_title('Median per hour', size=15)
...
axs[2, 0].set_title('Median per day', size=15)
axs[2, 0].set_xlabel('Milliseconds')
axs[2, 0].set_ylabel('Answers')

poly = get_poly(data['rawMedianDay_ts'], raw_answers)
pts = poly.get_path().vertices
...
axs[2, 1].set_title('Median per day', size=15)
...
#fig.show()
plt.savefig("cachedMedianDief.png", dpi=100)


f = open("cachedMedianDiefGraph_areas.txt", "w+")
f.write("Server Side: \n")
